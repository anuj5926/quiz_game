...
int main()
{
	system("color 3f");//color command of terminal to make background aqua and text white
...
mainhome:
	...
	printf("\n\t\t > press Q to Quit");
	printf("\n\n\t\t > Enter Your Choice : ");
	choice = toupper(getch());//getting the character from user without printing on screen and making it uppercase
	if (choice == 'V')
	{
		show_record();
		goto mainhome;
	...
	home:
		...
		if (count >= 2)//checking for eligibility
		{
			goto test;
		}
		else
		{
			...
			getch();
			goto mainhome;//going back to main screen
		}
	test:
		system("cls");
